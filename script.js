...
const DEFAULT_PROJECTS = [{
    id: "PRJ-001",
    name: "Cyberpunk RPG",
    description: "A futuristic role-playing game with high-quality 2D, 3D, animation, audio, interface, and cinematic assets.",
    manager: "Alex Cruz",
    status: "Active",
    created: "Sep 10, 2026"
}];
...
const DEFAULT_USERS = [
    { name: "Alex Cruz",    role: "Manager" },
    { name: "Maria Santos", role: "Artist" },
    { name: "John Doe",     role: "Artist" },
    { name: "Liam Evans",   role: "Artist" }
];

const DEFAULT_ASSETS = [
    { id: "AST-001", name: "hero_run_spritesheet", type: "Animation", project: "Cyberpunk RPG",
      artist: "Maria Santos", version: "v2", status: "Under Review", integration: "Not Ready", date: "Sep 19, 2026" },
    { id: "AST-002", name: "laser_blast_sfx", type: "Audio", project: "Cyberpunk RPG",
      artist: "John Doe", version: "v1", status: "Approved", integration: "Ready", date: "Sep 19, 2026" },
    { id: "AST-003", name: "boss_mech_model", type: "3D Mesh", project: "Cyberpunk RPG",
      artist: "Maria Santos", version: "v3", status: "In Progress", integration: "Not Ready", date: "Sep 19, 2026" },
    { id: "AST-004", name: "intro_cinematic_draft", type: "Cutscene", project: "Cyberpunk RPG",
      artist: "Liam Evans", version: "v1", status: "Under Review", integration: "Not Ready", date: "Sep 19, 2026" }
];

const DEFAULT_LOGS = [
    { ts: "2026-09-19T09:42:00", user: "Maria Santos", action: "Uploaded",  project: "Cyberpunk RPG", asset: "boss_mech_model",       version: "v3", result: "Success" },
    { ts: "2026-09-19T09:31:00", user: "Alex Cruz",    action: "Approved",  project: "Cyberpunk RPG", asset: "laser_blast_sfx",       version: "v1", result: "Success" },
    { ts: "2026-09-19T09:12:00", user: "Liam Evans",   action: "Submitted", project: "Cyberpunk RPG", asset: "intro_cinematic_draft", version: "v1", result: "Success" }
];
...
const DEFAULT_INTEGRATION = {
    ...
    lastTrigger: "Sep 19, 2026, 9:31 AM",
...
/* fill the Project / Artist dropdowns from the stored data */
function populateSelects() {
    const projectSel = $("assetProject");
    const artistSel = $("assetArtist");

    if (projectSel) {
        projectSel.innerHTML = getProjects()
            .map(p => `<option>${esc(p.name)}</option>`).join("");
    }

    if (artistSel) {
        const artists = getUsers().filter(u => u.role === "Artist");
        artistSel.innerHTML = (artists.length ? artists : getUsers())
            .map(u => `<option>${esc(u.name)}</option>`).join("");
    }
}
...
function registerAsset() {
    ...
    const artist = $("assetArtist").value;
...
function applyProfile() {
    const name = getSettings().profile.name;
